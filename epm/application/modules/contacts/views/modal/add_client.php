			 ...
			 <div class="form-group">
				...
				<div class="input-group">
                          ...
                          <span class="input-group-btn">
                            <button class="btn btn-default" id='check_username' type="button"><?=lang('check')?></button>
                          </span>
                        </div>
				...
				<div class="input-group">
                          ...
                          <span class="input-group-btn">
                            <button class="btn btn-default" id='check_email' type="button"><?=lang('check')?></button>
                          </span>
                        </div>
...
<script type="text/javascript">
    $(document).ready(function() {  
    	jQuery('#validation_check').addClass('hide');      
            ...
            var min_chars = 4;      
            //result texts  
            var characters_error = '<?=lang('minimum_amount_of_chars')?> '+min_chars;  
            var checking_html = '<?=lang('checking')?>';  
    ...
    function check_username_availability(){ 
            ...
            $.post("<?=base_url()?>contacts/username_check", { username: username },  
                function(result){   
                    if(result == 1){   
                        $('#validation_check').html('<i class="fa fa-check"></i> ' + username + ' <?=lang('is_available')?>');
                    }else{  
                        $('#validation_check').html('<i class="fa fa-warning"></i> ' + username + ' <?=lang('is_not_available')?>');  
                    }  
            });  
    ...
    function check_email_availability(){ 
            ...
            $.post("<?=base_url()?>contacts/email_check", { email: email },  
                function(result){   
                    if(result == 1){   
                        $('#validation_check').html('<i class="fa fa-check"></i> ' + email + ' <?=lang('does_not_exist')?>');  
                    }else{  
                        $('#validation_check').html('<i class="fa fa-warning"></i> ' + email + ' <?=lang('already_exists')?>');  
                    }  
            });  
