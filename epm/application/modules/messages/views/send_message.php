    ...
    <section class="hbox stretch">
        ...
        <aside class="bg-light lter" id="email-list">
            ...
            <?php if($this->tank_auth->get_role_id() == '1'){ ?>
                                                    ...
                                                    <optgroup label="<?=lang('clients')?>">
                                                        <?php foreach ($clients as $client): ?>
                                                            ...
                                                            <?=ucfirst(Applib::displayName($client->id))?></option>
                                                        <?php endforeach; ?>
                                                    </optgroup>
                                                    <optgroup label="<?=lang('administrators')?>">
                                                        <?php foreach ($admins as $admin): ?>
                                                            <option value="<?=$admin->id?>">
                                                            <?=ucfirst(Applib::displayName($admin->id))?></option>
                                                        <?php endforeach; ?>
                                                    </optgroup>
                                                    <optgroup label="<?=lang('staff')?>">
                                                        <?php foreach ($staff as $s): ?>
                                                            <option value="<?=$s->id?>">
                                                            <?=ucfirst(Applib::displayName($s->id))?></option>
                                                        <?php endforeach; ?>
                                                    </optgroup>
            ...
            <?php }else{ ?>
                                                    <optgroup label="<?=lang('administrators')?>">
                                                        <?php foreach ($admins as $admin): ?>
                                                            <option value="<?=$admin->id?>"><?=ucfirst(Applib::displayName($admin->id))?></option>
                                                        <?php endforeach; ?>
                                                    </optgroup>
            <?php } ?>
                                ...
                                <div class="panel-footer">
                                    <div class="text-center">
                                        <button type="submit" class="btn btn-sm btn-<?=config_item('theme_color');?>"><?=lang('send_message')?></button>
                                    </div>
                                </div>
                            </section>
                        </form>
