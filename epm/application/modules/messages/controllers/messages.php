		...
		case 'trash':
				$data['messages'] = $this->db->where(
								array(
									'user_to' => $this->tank_auth->get_user_id(),
									'deleted' => 'Yes'
								))->get(Applib::$messages_table)->result();
				break;
	...
	function send()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			redirect($_SERVER['HTTP_REFERER']);
				...
				$data['editor'] = TRUE;
				$data['group'] = $this->group;
				$data['clients'] = Applib::retrieve(Applib::$user_table,array('role_id' => 2,'activated' => 1));
				$data['admins'] = Applib::retrieve(Applib::$user_table,array('role_id' => 1,'activated' => 1));
				$data['staff'] = Applib::retrieve(Applib::$user_table,array('role_id' => 3,'activated' => 1));
