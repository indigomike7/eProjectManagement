...
class Profile extends MX_Controller {
		...
		if ($this->form_validation->run() == FALSE) // validation hasn't been passed
		                {
		                    ...
		                    //redirect('profile/settings');
                                    ...
                                    $this->db->where('user_id', $this->tank_auth->get_user_id());
                                        $this->db->update(Applib::$profile_table, $this->input->post()); 
	...
	$data['profile'] = Applib::retrieve(Applib::$profile_table,array('user_id' =>$this->tank_auth->get_user_id()));
	...
	{		
		...
		if ($this->input->post()) {
							...
							$config['file_name'] = strtoupper('USER-'.$this->tank_auth->get_username()).'-CV';
							$config['overwrite'] = TRUE;
							...
							if ( ! $this->upload->do_upload())
									{
										$this->session->set_flashdata('response_status', 'error');
										$this->session->set_flashdata('message',"Error Upload CV");
										redirect("profile/settings");
									}
									else
									{
				...
				$this->db->where('id',$this->tank_auth->get_user_id())
						 ->set('cv',$data['file_name'] )
						 ->update("fx_users");


				$this->session->set_flashdata('response_status', 'success');
				$this->session->set_flashdata('message',"CV Uploaded Successfully");
				redirect('profile/settings');

					
			}else{
				$this->session->set_flashdata('response_status', 'error');
				$this->session->set_flashdata('message', "No CV Selected");
				redirect('profile/settings');
		}
	}
	function activities()
