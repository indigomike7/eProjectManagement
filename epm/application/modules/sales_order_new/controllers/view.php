...
//                    $this->load->view('modal/create_sales_order',$data);
			...
			if ($this->form_validation->run()) {		// validation ok
                $date = date_format(date_create_from_format(config_item('date_php_format'), $this->form_validation->set_value('so_date')), 'Y-m-d');
                    ...
                    $this->db->where('so_id',$so_id)->update('fx_sales_order', $update); 
...
//										$file_name = $this->profile_model->update_avatar($data['file_name']);
                        ...
                        $client_quotation_file=array("files"=>$data2['file_name'],"so_id"=>$so_id,"type"=>"client_quotation_file");
...
//                    echo "test";
            ...
            if ($this->input->post()) 
            {
                    ...
                    {
                        $sales_order_files=$this->AppModel->get_all_records($table = 'fx_sales_order_files',
                            $array = array(
                                    'so_id =' => $so_id,"type = "=>"sales_costing"),$join_table = '',$join_criteria = '','so_id');
                        $sales_order_files2=$this->AppModel->get_all_records($table = 'fx_sales_order_files',
                            $array = array(
                                    'so_id =' => $so_id,"type = "=>"client_po"),$join_table = '',$join_criteria = '','so_id');
                        if(count($sales_order_files)<1 ||count($sales_order_files2)<1 )
                        {
...
//										$file_name = $this->profile_model->update_avatar($data['file_name']);
                        ...
                        $client_quotation_file=array("files"=>$data2['file_name'],"so_id"=>$id,"type"=>"sales_costing");
                $this->db->insert("fx_sales_order_files",$client_quotation_file);
                        }
                    }
                        }
...
//										$file_name = $this->profile_model->update_avatar($data['file_name']);
                        ...
                        $client_quotation_file=array("files"=>$data2['file_name'],"so_id"=>$id,"type"=>"client_po");
                $this->db->insert("fx_sales_order_files",$client_quotation_file);
                        }
                    }
                        }
...
//										$file_name = $this->profile_model->update_avatar($data['file_name']);
                        ...
                        $client_quotation_file=array("files"=>$data2['file_name'],"so_id"=>$id,"type"=>"client_co");
                $this->db->insert("fx_sales_order_files",$client_quotation_file);
                        }
                    }
                        }
...
//										$file_name = $this->profile_model->update_avatar($data['file_name']);
                        ...
                        $quotation_file=array("files"=>$data2['file_name'],"so_id"=>$id,"type"=>"supplier_quotation");
...
//										$file_name = $this->profile_model->update_avatar($data['file_name']);
                        ...
                        $quotation_file=array("files"=>$data2['file_name'],"so_id"=>$id,"type"=>"supplier_po");
              $this->db->insert("fx_sales_order_files",$quotation_file);
                        }
                }
            }
				...
				$sales_order=$this->AppModel->get_all_records($table = 'fx_sales_order',
                    $array = array(
                            'so_id =' => $so_id),$join_table = '',$join_criteria = '','so_id');
				//echo  $sales_order[0]->so_client_id;
                $data['role'] = Applib::login_info($this->session->userdata('user_id'))->role_id;
                    $data['clients'] = $this->AppModel->get_all_records($table = 'fx_companies',
                    $array = array(
                            'co_id >' => '0'),$join_table = '',$join_criteria = '','co_id');
                    $data['clients2'] = $this->AppModel->get_all_records($table = 'fx_companies',
                    $array = array(
                            'co_id = ' => $sales_order[0]->so_client_id),$join_table = '',$join_criteria = '','co_id');
                    $data['assigned_to'] = $this->AppModel->get_all_records($table = 'fx_users',
                    $array = array(
                            'fx_roles.role =' => 'e_sales_admin'),$join_table = 'fx_roles',$join_criteria = 'fx_users.role_id=fx_roles.r_id','fx_users.id');
            ...
            if ($this->input->post()) 
            {
                ...
                $data["sales_order_items"]=$this->AppModel->get_all_records($table = 'fx_sales_order_items',
                    ...
                    ob_start();
                        ...
                        $i=1;
                        $total=0;
						if(count($data["sales_order_items"])>0)
						{
...
//                    ob_end_clean();
        ...
        function approve_sales_order()
        {
            ...
            {
                $this->load->view('modal/approve_sales_order',$data);
            }
            ...
            if ($this->input->post()) 
            {
                ...
                if ($this->form_validation->run() == FALSE)
                {
                    ...
                    $this->input->post('sales_order_item_id');
                        ...
                        redirect(base_url()."sales_order_new/view/item_details/".$so_id);
                }
            }
            else
            {
	...
	function received_po()
	{
            ...
            {
                $this->load->view('modal/received_po',$data);
            }
	}
	function edit_items()
	{
...
//                    $this->load->view('modal/create_sales_order',$data);
		if ($this->input->post()) {
			...
			// check form validation
                        ...
                        $this->form_validation->set_rules('so_date', 'Sales Order Date', 'required');
			//$this->form_validation->run();
//                        echo $this->form_validation->set_value('so_number');
//                        echo $this->input->post("bypass");
                        if($this->input->post("bypass"))
                            $status="3";
                        else {
			...
			if ($this->form_validation->run()) {	
                ...
                $date = date_format(date_create_from_format(config_item('date_php_format'), $this->form_validation->set_value('so_date')), 'Y-m-d');
                    ...
                    $this->db->insert('fx_sales_order', $insert); 
                                
                                $data_id=$this->db->query("select * from fx_sales_order order by so_id desc limit 0,1")->result();
                                                                                $id = $data_id[0]->so_id;
                                                                                
                              //                                                  echo count($_FILES['client_quotation_file']['name']);
//                                echo print_r($data_id);
                    for($i=0;$i<count($_FILES['client_quotation_file']['name']);$i++)
                    {
                    if(file_exists($_FILES['client_quotation_file']['tmp_name'][$i]) || is_uploaded_file($_FILES['client_quotation_file']['tmp_name'][$i])) {
//                        echo "test";
