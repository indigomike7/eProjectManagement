...
class Settings extends MX_Controller {
	...
	function index()
	{
		...
		if($settings == 'general') $data['countries'] = $this -> settings -> countries();
	...
	function add_currency(){
		if ($_POST) {
			if($this->db->where('code',$this->input->post('code'))->get('currencies')->num_rows() == '0'){
				$this -> db -> insert('currencies',$this->input->post());
				$this->session->set_flashdata('response_status', 'success');
	...
	function add_category(){
		if ($_POST) {
			if($this->db->where('cat_name',$this->input->post('cat_name'))->get('categories')->num_rows() == '0'){
				$this -> db -> insert('categories',$this->input->post());
				$this->session->set_flashdata('response_status', 'success');
	...
	function departments(){
		if ($_POST) {
			...
			unset($_POST['settings']);
			 ...
			 $this -> db -> insert('departments',$this->input->post());
	...
	function add_custom_field(){
		if ($_POST) {
			...
			$this -> db -> insert('fields',$this->input->post());
