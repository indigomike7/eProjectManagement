...
<a href="<?=base_url()?>projects/tasks/add/<?=$this->uri->segment(4)?>"  data-toggle="ajaxModal" title="<?=lang('add_task')?>" class="btn btn-sm btn-info pull-right"><?=lang('add_task')?></a> 
<a href="<?=base_url()?>projects/tasks/add_from_template/<?=$this->uri->segment(4)*1200?>"  data-toggle="ajaxModal" title="<?=lang('from_templates')?>" class="btn btn-sm btn-info"><?=lang('from_templates')?></a> 
 </header>
...
<ul class="list-group no-radius m-b-none m-t-n-xxs list-group-lg no-border"> 
		...
		<a href="#" class="thumb-sm pull-left m-r-sm"> <img src="<?=base_url()?>resource/avatar/<?=$this->user_profile->get_profile_details($task->added_by,'avatar')?>" class="img-circle"> </a>
			<a href="<?=base_url()?>projects/tasks/edit/<?=$task->t_id?>/<?=$task->project?>" data-toggle="ajaxModal" title="<?=lang('edit_task')?>" class="clear"> <small class="pull-right">
			<?php
												$ts = $task->date_added;
												...
												echo $when; 
												?>
												</small> <strong class="block"><?=ucfirst($this->user_profile->get_profile_details($task->added_by,'fullname'))?></strong> 
							<?php
...
<a href="<?=base_url()?>projects/tasks/edit/<?=$task->t_id?>/<?=$task->project?>" data-toggle="ajaxModal" title="<?=lang('edit_task')?>" class="btn btn-xs btn-dark"><i class="fa fa-edit text-white"></i></a>


<?php
if ($task->auto_progress == 'FALSE') { ?>

	<a href="<?=base_url()?>projects/tasks/pilot/on/<?=$task->t_id?>/<?=$task->project*8600?>"  data-original-title="<?=lang('auto_progress_on')?>" class="btn btn-xs btn-primary" data-toggle="tooltip" data-placement="top"> <i class="fa fa-rocket text-white"></i></a>
<?php }else{ ?>
<a href="<?=base_url()?>projects/tasks/pilot/off/<?=$task->t_id?>/<?=$task->project*8600?>"  data-original-title="<?=lang('auto_progress_off')?>" class="btn btn-xs btn-dark" data-toggle="tooltip" data-placement="top"> <i class="fa fa-power-off text-white"></i></a>
<?php } ?> 
