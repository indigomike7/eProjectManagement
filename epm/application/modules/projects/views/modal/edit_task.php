...
<?php if($role != 2){ 
	if($this->db->where(array('assigned_user'=>$logged_user,'task_assigned' => $task->t_id))->get('assign_tasks')->num_rows() > 0) { ?>
				<div class="form-group">
...
<?php } ?>
		...
		<button type="submit" class="btn btn-<?=config_item('theme_color');?>"><?=lang('edit_task')?></button>
		</form>
		<?php } } ?>
		</div>
	</div>
