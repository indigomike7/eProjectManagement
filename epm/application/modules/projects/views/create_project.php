		...
		<aside class="aside-md bg-white b-r" id="subNav">
		 <header class="dk header b-b">
			...
			foreach ($projects as $key => $p) { 
				...
				<strong><?=$this->applib->fo_format_currency($p->currency, $project_cost)?></strong>
				<?php } ?>
				</small>
				</div> <br>
...
<?php if ($role == '1' OR $this->tank_auth->user_role($this->tank_auth->get_role_id()) == 'e_business' OR $this -> applib -> allowed_module('add_projects',$username)) { ?>
				<div class="form-group">
...
<?php if ($role == '1') { ?>
    			...
    			<optgroup label="<?=lang('admin_staff')?>"> 
					...
					foreach ($assign_to as $key => $user) { ?>
						<option value="<?=$user->id?>"><?=ucfirst(Applib::displayName($user->id))?></option>
					<?php } } ?>	
					</optgroup> 
    			</select>
				</div>
				...
				<div class="col-lg-3">
					<input type="text" class="form-control" placeholder="300" name="estimate_hours">
				</div>
				</div>
				...
				<div class="col-lg-9">
				<textarea name="description" class="form-control foeditor-100" placeholder="<?=lang('description')?>" required></textarea>
				</div>
				</div>
