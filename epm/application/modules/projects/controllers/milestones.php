...
class Milestones extends MX_Controller {
	...
	function add()
	{
		...
		if ($this->form_validation->run('projects','add_milestone') == FALSE)
		{
            ...
            $milestone_id = Applib::create(Applib::$milestones_table,$this->input->post());
            ...
            if(config_item('slack_notification') == 'TRUE'){
                ...
                $slack->slack_milestone_action($project,$milestone_id,$this->user,'added');
            }
	...
	function edit()
	{
		...
		if ($this->form_validation->run('projects','add_milestone') == FALSE)
		{
			...
			$milestone = $_POST['id'];
                        $_POST['start_date'] = date_format(date_create_from_format(config_item('date_php_format'), $_POST['start_date']), 'Y-m-d');
                        $_POST['due_date'] = date_format(date_create_from_format(config_item('date_php_format'), $_POST['due_date']), 'Y-m-d');
            Applib::update(Applib::$milestones_table,array('id'=>$milestone),$this->input->post());
            ...
            if(config_item('slack_notification') == 'TRUE'){
            	$this->load->helper('slack');
            	$slack = new Slack_Post;
                $slack->slack_milestone_action($project,$milestone,$this->user,'edited');
            }
