...
//                    $path = date("Y-m-d",  strtotime($p->date_created))."_".$project."_".$p->project_code.'/';
                    $path = $p->project_code.'/'.$this->tank_auth->user_role($this->tank_auth->get_role_id())."/".$this->username;
	...
	function _upload_notification($project){
			...
			$message = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'project_file'), 'template_body');
                        $subject = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'project_file'), 'subject');
