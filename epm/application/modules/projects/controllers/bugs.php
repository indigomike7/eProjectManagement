...
class Bugs extends MX_Controller {
    ...
    function add() {
        if ($this->input->post()) {
            ...
            $bug_id = Applib::create(Applib::$bugs_table, $this->input->post());
            // log activity
            $this->_log_activity(
                                        'activity_issue_added', 
                                        $this->user, 
                                        'bugs', 
                                        $bug_id, 
                                        'fa-bug', 
                                        $this->input->post('issue_title'), 
                                        ''
                                );

            if (config_item('notify_bug_assignment') == 'TRUE') {
    ...
    function edit() {
        if ($this->input->post()) {
            ...
            Applib::update(Applib::$bugs_table,array('bug_id' => $bug_id),$this->input->post());

            if (config_item('notify_bug_assignment') == 'TRUE' AND $this->user_role == '1') {
                $this->_notify_assigned_bug($_POST['bug_id']);
            }
    ...
    function _upload_notification($bug) {
        ...
        $message = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'bug_file'), 'template_body');
        $subject = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'bug_file'), 'subject');
        ...
        $uploaded_by = str_replace("{UPLOADED_BY}", $this->username, $logo);
    ...
    function status() {
        ...
        // Post to slack channel
            if(config_item('slack_notification') == 'TRUE'){
                ...
                $issue_title = $this->db->where('bug_id',$bug)->get('bugs')->row()->issue_title;
                Applib::slack(
                          sprintf(lang('slack_bug_status_msg'), $issue_title, strtoupper($status)),
                          sprintf(lang('slack_bug_status'), $project_title),
                          base_url().'projects/view/'.$project.'?group=bugs',
                          sprintf(lang('slack_bug_status_by'), Applib::profile_info($this->user)->fullname)
                          );
            }
    ...
    function _notify_bug_status_change($bug, $project, $status) {
        ...
        $message = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'bug_status'), 'template_body');
        $subject = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'bug_status'), 'subject');
        $signature = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'email_signature'), 'template_body');
        ...
        $email_issue_title = str_replace("{ISSUE_TITLE}", $issue_title, $logo);
        ...
        $EmailSignature = str_replace("{SIGNATURE}",$signature,$bug_url);
        $message = str_replace("{SITE_NAME}", config_item('company_name'), $EmailSignature);
    ...
    function comment() {
        if ($this->input->post()) {
            ...
            $comment_id = Applib::create(Applib::$bug_comments_table, $this->input->post());
            ...
            if (config_item('notify_bug_comments') == 'TRUE') {
                ...
                // Notify reporter 
                    $this->_notify_bug_comment($bug, $project, 'reporter',$comment);
                    // Notify staff
                    $this->_notify_bug_comment($bug, $project, 'staff',$comment);
    ...
    function _notify_bug_comment($bug, $project, $notify, $comment) {

        $message = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'bug_comment'), 'template_body');
        $subject = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'bug_comment'), 'subject');
        $signature = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'email_signature'), 'template_body');
    ...
    function _notify_assigned_bug($bug) {
        ...
        $message = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'bug_assigned'), 'template_body');
        $subject = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'bug_assigned'), 'subject');
        $signature = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'email_signature'), 'template_body');
        ...
        $issue_title = str_replace("{ISSUE_TITLE}", $issue_title, $logo);
    ...
    function _reported_notification($bug) {
        ...
        $message = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'bug_reported'), 'template_body');
        $subject = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'bug_reported'), 'subject');
        $signature = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'email_signature'), 'template_body');

        $logo_link = '<img style="width:300px" src="'.base_url().'/resource/images/logos/'.config_item('invoice_logo').'"/>';
            
        $logo = str_replace("{INVOICE_LOGO}",$logo_link,$message);
        $title = str_replace("{ISSUE_TITLE}", $issue_title, $logo);
        ...
        $client = Applib::get_table_field('projects', array('project_id' => $project), 'client');

        $params['recipient'] = $this->db->where('co_id',$client)->get('companies')->row()->company_email;
