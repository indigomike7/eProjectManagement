...
class Timesheet extends MX_Controller {
	...
	function billable(){
		if (isset($_GET['id'])) {
			...
			if ($_GET['cat'] == 'tasks') { $tbl = 'tasks_timer'; } else{ $tbl = 'project_timer'; }
			
			$billable = $this->db->select('timer_id,billable')
								 ->where('timer_id',$_GET['id'])
								 ->get($tbl)
								 ->row()
								 ->billable;
			if ($billable == '1') {
				$args = array('billable' => 0);
				$this->db->where('timer_id',$_GET['id'])->update($tbl,$args);
				$this -> applib -> redirect_to($_SERVER['HTTP_REFERER'],'success',lang('operation_successful'));
			}else{
	...
	function description(){
		...
		$data['description'] = $this->db->where('timer_id',$timer_id)->get($tbl)->row()->description;
		$this->load->view('modal/timer_description',$data);
	}
