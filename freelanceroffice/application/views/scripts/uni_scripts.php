...
<script type="text/javascript">
        ...
        "aoColumnDefs":[{
            ...
            $("#table-tickets").dataTable().fnSort([[1,'desc']]);
            ...
            $("#table-expenses").dataTable().fnSort([[5,'desc']]);
...
$(".gantt").gantt({
    ...
    <?php
    if($role != '2'){
    $tasks = $this->db->where('project',$project)->get('tasks')->result();
    }else{
    $tasks = $this->db->where(array('project'=>$project,'visible'=>'Yes'))->get('tasks')->result();
    }
    foreach ($tasks as $key => $t) { ?>
{
  "name": '<a href="<?=site_url()?>projects/view/<?=$project?>?group=tasks&view=task&id=<?=$t->t_id?>" class="text-info"><?=$t->task_name?> </a>',
  "desc": "",
  "values": [
            {"from":  Date.parse("<?=date('Y/m/d',strtotime($t->date_added))?>"), "to": Date.parse("<?=date('Y/m/d',strtotime($t->due_date))?>"),
            "desc": "<b><?=$t->task_name?></b> - <em><?=$t->task_progress?>% <?=lang('done')?></em><br><div class=\"line line-dashed line-lg pull-in\"></div><em><?=lang('start_date')?>: <span class=\"text-success text-small\"><?=date('Y-m-d',strtotime($t->date_added))?></span> to <?=lang('due_date')?>: <span class=\"text-danger text-small\"><?=$t->due_date?></span></em>",
