...
class Items extends MX_Controller {
	...
	function add(){
		...
		{	
			...
			if ($tax_rate == '0.00') {
				$tax_rate = $this->db
								 ->where('item_name',$this->input->post('item_name'))
								 ->get('items_saved')
								 ->row()
								 ->item_tax_rate;
			}
			$_POST['item_tax_rate'] = $tax_rate;
			$_POST['item_tax_total'] = ($_POST['item_tax_rate'] / 100) *  $sub_total;
			$_POST['total_cost'] = $sub_total + $_POST['item_tax_total'];
			unset($_POST['tax']);

				if($this -> items -> add($this->items_table,$this->input->post())){
	...
	function edit(){
		...
		$invoice_id = $this->applib->get_table_field($this->items_table, array('item_id'=>$_POST['item_id']),'invoice_id');
		if ($this -> form_validation -> run('invoices','add_item') == FALSE)
		{	
				$_POST = '';
				$this -> applib -> redirect_to('invoices/view/'.$invoice_id,'error',lang('error_in_form'));	
			...
			$_POST['total_cost'] = $sub_total + $_POST['item_tax_total'];
				...
				if(Applib::update(Applib::$invoice_items_table, array('item_id' => $_POST['item_id']),$this->input->post())){
					$this -> applib -> redirect_to('invoices/view/'.$invoice_id,'success',lang('item_added_successfully'));
				}
			}
		}else{
