...
<section id="content">
    ...
    <section class="hbox stretch">
        ...
        <?php if ($role == '1' OR $this->applib->allowed_module('edit_all_invoices', $username)) : ?>
                            ...
                            <a href="<?= base_url() ?>invoices/items/insert/<?= $inv->inv_id ?>" title="<?= lang('item_quick_add') ?>" class="btn btn-sm btn-dark" data-toggle="ajaxModal">
                            ...
                            <a class="btn btn-sm btn-<?=config_item('theme_color')?>" href="<?= base_url() ?>invoices/hide/<?= $inv->inv_id ?>" data-toggle="tooltip" data-original-title="<?= lang('hide_to_client') ?>" data-placement="bottom">
                            <i class="fa fa-eye-slash"></i> 
                            </a>
                            <?php } else { ?>


                            <a class="btn btn-sm btn-dark" href="<?= base_url() ?>invoices/show/<?= $inv->inv_id ?>" data-toggle="tooltip" data-original-title="<?= lang('show_to_client') ?>" data-placement="bottom"><i class="fa fa-eye"></i> 
                            </a>
        ...
        <?php endif; ?>
         ...
         <?php if (round($this -> applib -> calculate('invoice_due',$inv->inv_id),2) > 0) : ?>

                            
        <?php if ( $role == '1' 
                OR $this->applib->allowed_module('pay_invoice_offline', $username) 
                AND round($this -> applib -> calculate('invoice_due',$inv->inv_id),2) > 0 
                ) {
        ?>

                            <a class="btn btn-sm btn-<?=config_item('theme_color');?>" href="<?= base_url() ?>invoices/pay/<?= $inv->inv_id ?>" data-toggle="tooltip" data-original-title="<?= lang('pay_invoice') ?>" data-placement="bottom">
        ...
        <?php } else { 
            if ($inv->allow_paypal == 'Yes') : ?>
                            ...
                            <a class="btn btn-sm btn-<?=config_item('theme_color')?>" href="<?= base_url() ?>paypal/pay/<?= $inv->inv_id ?>" data-toggle="ajaxModal" title="<?= lang('via_paypal') ?>"><?= lang('via_paypal') ?>
                            </a>
        ...
        <?php if ($inv->allow_stripe == 'Yes') : ?>
                            ...
                            <button id="customButton" class="btn btn-sm btn-<?=config_item('theme_color')?>" >
                            <?=lang('via_stripe')?></button>

        <?php endif; ?>
        <?php if ($inv->allow_bitcoin == 'Yes') : ?>

                            <a class="btn btn-sm btn-<?=config_item('theme_color')?>" href="<?= base_url() ?>bitcoin/pay/<?= $inv->inv_id ?>" data-toggle="ajaxModal" title="<?= lang('via_bitcoin') ?>"><?= lang('via_bitcoin') ?>
                            </a>
    ...
    <?php endif; ?>
        ...
        <?php
        if ($role == '1' AND $inv->recurring == 'Yes' 
                OR $this->applib->allowed_module('edit_all_invoices', $username)) { 
            ?>


                            <a class="btn btn-sm btn-dark" href="<?= base_url() ?>invoices/stop_recur/<?= $inv->inv_id ?>"
        ...
        <?php if ($role == '1' 
                OR $this->applib->allowed_module('edit_all_invoices', $username)) { ?>
                    ...
                    <a href="<?= base_url() ?>invoices/edit/<?= $inv->inv_id ?>" class="btn btn-sm btn-<?=config_item('theme_color')?>" data-original-title="<?=lang('edit_invoice')?>" data-toggle="tooltip" data-placement="bottom">
        ...
        <?php if ($role == '1' AND round($this -> applib -> calculate('invoice_due',$inv->inv_id),2) > 0) : ?>

                                    <li>
                                    <a href="<?= base_url() ?>invoices/mark_as_paid/<?= $inv->inv_id ?>" data-toggle="ajaxModal"><?= lang('mark_as_paid') ?>
                                    </a>
                                    </li>
        ...
        <?php if ($role == '1' OR $this->applib->allowed_module('send_email_reminders', $username)) : ?>
                                    ...
                                    <li>
                                        <a href="<?= base_url() ?>invoices/remind/<?= $inv->inv_id ?>" data-toggle="ajaxModal" title="<?= lang('send_reminder') ?>"><?= lang('send_reminder') ?>
                                        </a>
                                    </li>
        <?php endif; ?>
                ...
                <?php endif; ?>
                    ...
                    <section class="scrollable wrapper">
                        <div class="row">
                            ...
                            <div>
                                <?= lang('invoice_date') ?><span class="col-xs-3 no-gutter-right pull-right"><strong><?= strftime(config_item('date_format'), strtotime($inv->date_saved)); ?></strong></span>
                            </div>

                            <?php if ($inv->recurring == 'Yes') { ?>
                            <div>
                                <?= lang('recur_next_date') ?><span class="col-xs-3 no-gutter-right pull-right"><strong><?= strftime(config_item('date_format'), strtotime($inv->recur_next_date)); ?></strong></span>
                            </div>
                            <?php } ?>

                            <div>
                            <?= lang('payment_due') ?><span class="col-xs-3 no-gutter-right pull-right"><strong><?= strftime(config_item('date_format'), strtotime($inv->due_date)); ?></strong></span>
                            </div>
                            ...
                            <div class="col-xs-6">
                                ...
                                <p>
                                    ...
                                    <span class="col-xs-9 no-gutter">
                                        ...
                                        <?php if ($this->applib->company_details($inv->client, 'state') != '') { echo $this->applib->company_details($inv->client, 'state').", ";  } ?>
                                    ...
                                    <?php if($this->applib->company_details($inv->client, 'VAT') != ''){ ?>
                                    <span class="col-xs-3 no-gutter"><?= lang('company_vat') ?>:</span>
...
<?php if ($role == '1' || $this->applib->allowed_module('edit_all_invoices', $username)) { ?>
<?php if(Applib::clientExpenses($inv->client) > 0) { ?>
