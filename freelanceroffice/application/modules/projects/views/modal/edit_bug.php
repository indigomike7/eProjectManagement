	...
	<div class="modal-content">
		...
		<div class="modal-body">
			 ...
			 <input type="hidden" name="project" value="<?=$i->project?>">
				...
				<option value="<?=$i->reporter?>" selected="selected"><?=ucfirst(Applib::displayName($i->reporter))?></option>
					<?php
				...
				<option value="-"><?=lang('not_assigned')?></option>				
				 ...
				 foreach ($assign_to as $value) { ?>
				 	<option value="<?=$value->id?>"<?=($i->assigned_to == $value->id ? ' selected="selected"' : '')?>><?=ucfirst(Applib::displayName($value->id))?></option>
                            <?php } ?>
                    <?php } ?>							
				</select>
				</div>
		...
		<button type="submit" class="btn btn-<?=config_item('theme_color');?>"><?=lang('edit_bug')?></button>
		</form>
		<?php } } ?>
		</div>
	</div>
