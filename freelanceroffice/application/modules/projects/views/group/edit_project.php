...
<div class="col-sm-12">
	<section class="panel panel-default">
		...
		<div class="panel-body">
			...
			if ($role == '1' OR $this -> applib -> allowed_module('edit_all_projects',$username) or $this->tank_auth->user_role($this->tank_auth->get_role_id()) == 'e_project_management' or $this->tank_auth->user_role($this->tank_auth->get_role_id()) == 'e_business'){
...
//                                                                        echo print_r($assign_to);

?>
                                                                    <select class="select2-option" multiple="multiple" style="width:260px" name="assign_to[]" > 
										<optgroup label="Staff">
											<?php 
											if (!empty($assign_to) && (is_array($assign_to) || is_object($assign_to))) {
												foreach ($assign_to as $user):
												echo '<option value="'.$user->id.'"';
                                                                                                if(is_array($project->assign_to) || is_object($project->assign_to)) 
                                                                                                { 
                                                                                                    $assigned=unserialize($project->assign_to); 
                                                                                                ...
                                                                                                echo ">";
                                                                                                echo ucfirst(Applib::displayName($user->id));
                                                                                                ?>
												</option>
											<?php endforeach ?>
											<?php } ?>	
