...
class Tasks extends MX_Controller {
    ...
    function edit() {
        if ($this->input->post()) {
            ...
            if(config_item('slack_notification') == 'TRUE'){
                ...
                $slack->slack_task_action($project,$task_id,$this->user,'edited');
            }
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run('projects', 'add_task') == FALSE) {
                ...
                $this->applib->redirect_to('projects/view/' . $project . '?group=tasks', 'error', lang('task_add_failed'));
                 ...
                 if($this->input->post('estimate') == ''){ $est_time = 0; }else{ $est_time = $_POST['estimate']; }
            ...
            if(config_item('slack_notification') == 'TRUE'){
                $this->load->helper('slack');
                $slack = new Slack_Post;
                $slack->slack_task_action($project,$task_id,$this->user,'added');
            }
    ...
    function file() {
        ...
        if ($this->input->post()) {
            ...
            if(config_item('slack_notification') == 'TRUE'){
                ...
                $task_name = $this->db->where('t_id',$task)->get('tasks')->row()->task_name;
                Applib::slack(
                          sprintf(lang('task_file_msg'), $fileinf['file_name'], $task_name),
                          sprintf(lang('new_task_file'), $project_title),
                          base_url().'projects/view/'.$project.'?group=tasks',
                          sprintf(lang('file_uploaded_by'), Applib::profile_info($this->user)->fullname)
                          );
            }
    ...
    function progress(){
        if($_POST){
            ...
            $this->db->where('t_id',$task_id)->update('tasks',$args);
    ...
    function _task_changed_notification($project, $task_name, $assigned_to) {
        ...
        $task_name = str_replace("{TASK_NAME}", $task_name, $logo);
    ...
    function _assigned_notification($project, $task_name, $assigned_to) {
        ...
        $message = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'task_assigned'), 'template_body');
        $subject = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'task_assigned'), 'subject');
        $signature = Applib::get_table_field(Applib::$email_templates_table,array('email_group' => 'email_signature'), 'template_body');
