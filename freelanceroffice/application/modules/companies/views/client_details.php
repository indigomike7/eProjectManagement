<?php $this->applib->set_locale(); ?>
	...
	<a href="<?=base_url()?>companies/view/edit/<?=$i->co_id?>" class="btn btn-<?=config_item('theme_color');?> btn-sm pull-right" data-toggle="ajaxModal" title="<?=lang('edit')?>"><i class="fa fa-edit"></i> <?=lang('edit')?></a>
	...
	<div class="col-md-6"><strong><?php if ($i->individual == 0) { echo lang('company_name'); } else { echo lang('full_name'); } ?></strong></div>
	<div class="col-md-6"><?=$i->company_name?></div>
	</div>
	<?php if ($i->individual == 0) { ?>
	<div class="row inline-fields">
	<div class="col-md-6"><strong><?=lang('contact_person')?></strong></div>
	<div class="col-md-6"><?=($i->primary_contact) ? Applib::displayName($i->primary_contact) : ''; ?></div>
	</div>
	<?php } ?>
	<div class="row inline-fields">
...
<?=lang('expense_cost')?> : 
	...
	<?php $cur = $this->applib->client_currency($i->co_id);?>
	<?=$this->applib->fo_format_currency($cur->code, Applib::clientExpenses($i->co_id))?>
	</strong>
