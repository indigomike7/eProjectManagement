			...
			foreach ($estimate_items as $key => $est_item) {
				$items_data = array(
			                ...
			                'quantity' => $est_item->quantity,
											'item_tax_rate' => $est_item->item_tax_rate,
											'item_tax_total' => $est_item->item_tax_total,
			                'total_cost' => $est_item->total_cost,
			            );
				$this->db->insert($this->invoice_items_table, $items_data);
			}

			$this->_log_activity('activity_estimate_converted',$this->tank_auth->get_user_id(),'estimates',$id,$icon = 'fa-laptop',$i->reference_no, $ref); //log activity
