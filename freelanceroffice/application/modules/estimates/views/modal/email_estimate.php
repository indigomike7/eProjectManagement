<div class="modal-dialog">
	...
	foreach ($estimate_details as $key => $estimate) { ?>
		...
		<div class="modal-body">
			...
			<input type="hidden" name="amount" value="<?=number_format($this -> applib -> est_calculate('estimate_amount',$estimate->est_id),config_item('currency_decimals'),config_item('decimal_separator'),config_item('thousand_separator'))?>">
		...
		<button type="submit" class="submit btn btn-<?=config_item('theme_color');?>"><?=lang('email_estimate')?></button>
		</form>
		</div>
	</div>
	<?php } ?>
