  ...
  <section class="scrollable wrapper">
    ...
    <div class="row">
      ...
      <div class="panel-body">
        ...
        <?php
        if ($p->company > 0) {
            $company = $this->db->where('co_id',$p->company)->get('companies')->result();
            $comp = $company[0];
        ?>  
         <div class="form-group">
     ...
     <div class="form-group">
      ...
      <h4 class="page-header">Curriculum Vitae</h4>

       <?php
       $attributes = array('class' => 'bs-example form-horizontal');
        echo form_open_multipart(base_url().'profile/upload_cv',$attributes); 
        $data=$this->db->where("id",$this->tank_auth->get_user_id())->get("fx_users")->result();
        $cv_now=$data[0]->cv;
        ?>
        <input type="hidden" name="r_url" value="<?=uri_string()?>">
       ...
       <div class="form-group">
        ...
        <div class="col-lg-9">
          ...
          <?php echo "<a href='".  base_url()."user/".$this->tank_auth->get_username().'/'.$data[0]->cv."'>".$data[0]->cv."</a>"; ?>
        </div>
      </div>
        <button type="submit" class="btn btn-sm btn-success">Upload CV</button>
      </form>
