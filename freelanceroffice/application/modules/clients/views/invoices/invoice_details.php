						...
						<header class="header bg-white b-b clearfix hidden-print">
							...
							<section class="scrollable wrapper w-f">
								...
								<section class="scrollable wrapper">
									<div class="row">
										<div class="col-xs-6">
											<p class="h4"><strong><?=config_item('company_name')?></strong></p>
											
											<?=config_item('company_address')?><br>
											<?=config_item('company_city')?><br>
											<?=config_item('company_country')?><br>
											<?=lang('phone')?>: <a href="tel:<?=config_item('company_phone')?>"><?=config_item('company_phone')?></a><br>
											<?=lang('company_vat').': '.config_item('company_vat')?>
							...
							<?=lang('vat')?> : <?=ucfirst($this->applib->company_details($inv->client,'VAT'))?> <br>
									...
									<table class="table"><thead>
										...
										<tr>
											...
											<td><?=$this->applib->fo_format_quantity($item->quantity)?></td>
											<td><?=$this->applib->fo_format_currency($inv->currency, $item->unit_cost)?></td>
											<td><?=$this->applib->fo_format_currency($inv->currency, $item->total_cost)?>
											</td>
										</tr>
										<?php } } ?>
										...
										<tr>
											...
											<td class="text-right"><?=$this->applib->fo_format_currency($inv->currency, $invoice_cost)?></td>
										</tr>
										<tr>
											<td colspan="3" class="text-right no-border"><strong><?=lang('tax')?> - <?=$this->applib->fo_format_tax($inv->tax)?>%</strong></td>
											<td class="text-right"><?=$this->applib->fo_format_currency($inv->currency, $tax)?></td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td class="text-right"><?=$this->applib->fo_format_currency($inv->currency, ($invoice_cost + $tax) - $payment_made)?></td>
										</tr>
									</tbody>
								</table>
							</section>
