...
abstract class REST_Controller extends MX_Controller
{
    ...
    */
    ...
    public function __construct()
    {
        ...
        if (config_item('rest_ip_blacklist_enabled')) {
	...
	// Skip keys test for $config['auth_override_class_method']['class'['method'] = 'none'
        ...
        if ($this->auth_override !== true && !(config_item('rest_enable_keys') && $this->_allow === true)) {
            $rest_auth = strtolower(config_item('rest_auth'));
            switch ($rest_auth) {
                case 'basic':
    ...
    public function response($data = null, $http_code = null, $continue = false)
    {
        ...
        // Otherwise (if no data but 200 provided) or some data, carry on camping!
            ...
            if (config_item('compress_output') === true && $this->_zlib_oc == false) {
            ...
            if (method_exists($this, '_format_'.$this->response->format)) {
                ...
                header('Content-Type: '.$this->_supported_formats[$this->response->format] . '; charset=' . strtolower(config_item('charset')));
        ...
        if ( ! $this->_zlib_oc && ! config_item('compress_output')) {
    ...
    protected function _detect_output_format()
    {
        ...
        if (config_item('rest_ignore_http_accept') === false and $this->input->server('HTTP_ACCEPT')) {
            // Check all formats against the HTTP_ACCEPT header
    ...
    protected function _perform_ldap_auth($username = '', $password = null)
    {
        ...
        $ldaptimeout = config_item('timeout', 'ldap');
        $ldaphost = config_item('server', 'ldap');
        $ldapport = config_item('port', 'ldap');
        $ldaprdn = config_item('binduser', 'ldap');
        $ldappass = config_item('bindpw', 'ldap');
        $ldapbasedn = config_item('basedn', 'ldap');

        log_message('debug', 'LDAP Auth: Connect to ' . $ldaphost);

        $ldapconfig['authrealm'] = config_item('domain', 'ldap');
    ...
    protected function _perform_library_auth($username = '', $password = null)
    {
        ...
        $auth_library_class     = strtolower(config_item('auth_library_class'));
        $auth_library_function  = strtolower(config_item('auth_library_function'));
    ...
    protected function _check_login($username = '', $password = false)
    {
        ...
        $auth_source = strtolower(config_item('auth_source'));
        $rest_auth = strtolower(config_item('rest_auth'));
        $valid_logins = config_item('rest_valid_logins');
        
        if (!config_item('auth_source') && $rest_auth == 'digest') { // for digest we do not have a password passed as argument
            return md5($username.':'.config_item('rest_realm').':'.(isset($valid_logins[$username])?$valid_logins[$username]:''));
        }
    ...
    {
        if (strtolower( config_item('rest_auth') ) == 'basic') {
            header('WWW-Authenticate: Basic realm="'.config_item('rest_realm').'"');
        } elseif (strtolower( config_item('rest_auth') ) == 'digest') {
            header('WWW-Authenticate: Digest realm="'.config_item('rest_realm').'", qop="auth", nonce="'.$nonce.'", opaque="'.md5(config_item('rest_realm')).'"');
        }
